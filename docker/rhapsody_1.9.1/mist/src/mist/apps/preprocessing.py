...
# filter gene based on list if provided
def filter_genes(X,genes,thresh=0,gene_list=None):
    ...
    if gene_list is not None:
        keep_inds_list = np.array([i for i in range(len(genes)) if genes[i] in gene_list])
        keep_inds = np.intersect1d(keep_inds,keep_inds_list)
        print('include %d genes in the gene list. %d genes in total'%(len(gene_list), len(keep_inds)))
...
def normalizecounts_multiplymedian(X):
...
def low_dimensional_embedding1(X,pcs=50):
    xpca = sk_pca(X,showplot=False)
    xtsne = sk_tsne(xpca[:,:pcs],showplot=False)
    return xtsne[:,0],xtsne[:,1]

# Low dimensional embedding using tSNE (bh_tsne package) and PCA
def low_dimensional_embedding2(X,pcs=50, per=30):
    xpca = sk_pca(X,showplot=False)
    xtsne = bh_sne(xpca[:,:pcs], perplexity=per)
    return xtsne[:,0],xtsne[:,1]
