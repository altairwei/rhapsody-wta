...
def add_to_bam(annot_r1,        # type: List[str]
    ...
    if 'Filtered' in cell_order_dict:
        cellList = set(cell_order_dict['Filtered'])
    else:
        cellList = set()
    ...
    molDict = findAdjustedMols(annot_mol_file, label_version, cellList)
    logging.info('...done')
    ...
    addR1toBAM(matching_R1, molDict, cellList, r2_bam, final_BAM, assay, label_version, tag_calls_df, target_gene_dict)
    logging.info('...done')
...
def findAdjustedMols(mol_annot_fp, label_version, cell_list):
    # type: (str, int) -> dict
    ...
    with utils.quick_gzip_open(mol_annot_fp) as f:
        ...
        for row in mol_annot_reader:
            cell_index, MI, gene, corrected_UMI = row[0], row[1], row[2], row[3]
...
def addR1toBAM(annotR1,                         # type: str
               ...
               target_gene_dict = None           # type: dict
               ):
    # type: (...) -> None
                ...
                if readAnnot_R1[3] == 'T':  # polyT
                    if (assay == 'WTA' and gene != '*') or \
                        ...
                        try:
                            ...
                            if target_gene_dict:
                                mol_corrected = umi_to_corrected_umi_df[(cell_index, target_gene_dict[gene], mol_label)]
                            else:
