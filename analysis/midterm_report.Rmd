...
```{r}
DEGs_1DPI <- ds_res$DESeq2 %>%
  rhapsodykit::diff_state_significant() %>%
  rhapsodykit::diff_state_format() %>%
  dplyr::filter(
    contrast %in% c("X1DPI.PNR2-X1DPI.MOCK", "X1DPI.TR4-X1DPI.MOCK"),
    cluster_id %in% c("VCs_5", "MCs_5")) %>%
  dplyr::select(gene, cluster_id, contrast, logFC, p_val, p_adj.loc, p_adj.glb)
...
DEGs_1DPI <- dplyr::left_join(DEGs_1DPI, anno, by = c("gene" = "ensembl_gene_id")) %>%
...
htmltools::browsable(
  htmltools::tagList(
    htmltools::tags$button(
      "Download as CSV",
      onclick = sprintf("Reactable.downloadDataCSV('%s', '%s')", "deg_1dpi_table", "deg_1dpi_table")
    ),
    reactable::reactable(
      DEGs_1DPI,
      defaultPageSize = 5,
      elementId = "deg_1dpi_table"
    )
  )
)
```
