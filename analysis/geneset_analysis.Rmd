...
##### PNR2 和 TR4 在 1DPI 诱导基因集的功能分析

要做富集分析我们首先要拿到一个基因列表。

```{r enrich-barplot, fig.height=7, fig.width=14}
plot_contrast_enrich_barplot <- function(result, contrast, cluster, title = NULL) {
  result %>%
    rhapsodykit::diff_state_significant() %>%
    rhapsodykit::diff_state_pull(contrast, cluster, "gene") %>%
    rhapsodykit::enrichment_analysis(go_data) %>%
    rhapsodykit::enrich_barplot(
      x = "Count", show_category = 20,
      title = ifelse(is.null(title), contrast, title)
    )
}

p1 <- plot_contrast_enrich_barplot(
  ds_res$DESeq2,
  "X1DPI.PNR2-X1DPI.MOCK", "VCs_5",
  "VCs_5 of 1DPI-PNR2 vs. 1DPI-MOCK"
)

p2 <- plot_contrast_enrich_barplot(
  ds_res$DESeq2,
  "X1DPI.TR4-X1DPI.MOCK", "VCs_5",
  "VCs_5 of 1DPI-TR4 vs. 1DPI-MOCK"
)

p3 <- plot_contrast_enrich_barplot(
  ds_res$DESeq2,
  "X1DPI.PNR2-X1DPI.MOCK", "MCs_5",
  "MCs_5 of 1DPI-PNR2 vs. 1DPI-MOCK"
)

p4 <- plot_contrast_enrich_barplot(
  ds_res$DESeq2,
  "X1DPI.TR4-X1DPI.MOCK", "MCs_5",
  "MCs_5 of 1DPI-TR4 vs. 1DPI-MOCK"
)
...
```{r cnetplot, fig.height=6, fig.width=12}
make_enrich_result <- function(ds, contrast, cluster) {
  sig_genes <- ds %>%
    rhapsodykit::diff_state_significant()
  genes <- sig_genes %>%
    rhapsodykit::diff_state_pull(contrast, cluster, "gene")
  fc <- sig_genes %>%
    rhapsodykit::diff_state_pull(contrast, cluster, "logFC")
  names(fc) <- genes
...
enr1 <- make_enrich_result(ds_res$DESeq2, "X1DPI.PNR2-X1DPI.MOCK", "VCs_5")
enr2 <- make_enrich_result(ds_res$DESeq2, "X1DPI.TR4-X1DPI.MOCK", "VCs_5")

rhapsodykit::enrich_cnetplot(enr1, fold_change = fc, show_category = 40) +
  rhapsodykit::enrich_cnetplot(enr2, fold_change = fc, show_category = 40)
```

如果我们不将基因显示为 node ，而是将 GO 词条之间的链接定义为重叠基因的数量，也就是说 GO 词条之间重叠基因越多他们在图上的距离也就越近。这个功能叫做 Enrichment Map 。

```{r emapplot, fig.height=4, fig.width=8}
p1 <- enr1 %>%
  enrichplot::pairwise_termsim() %>%
  enrichplot::emapplot(cex_label_category = 0.6, showCategory = 40)
p2 <- enr2 %>%
  enrichplot::pairwise_termsim() %>%
  enrichplot::emapplot(cex_label_category = 0.6, showCategory = 40)
...
p1 <- enr1 %>% enrichplot::upsetplot()
p2 <- enr2 %>% enrichplot::upsetplot()
...
```{r enrich-compare, fig.height=12, fig.width=5, fig.keep="last"}
compare_list <- list(
...
enrichplot::dotplot(comp_enr, showCategory = 40, by = "count", label_format = 60) +
...
```{r gsea-data}
make_gsea_result <- function(ds, contrast, cluster) {
...
gsea1 <- make_gsea_result(ds_res$DESeq2, "X1DPI.PNR2-X1DPI.MOCK", "VCs_5")
gsea2 <- make_gsea_result(ds_res$DESeq2, "X1DPI.TR4-X1DPI.MOCK", "VCs_5")
```
...
rhapsodykit::enrich_cnetplot(gsea1$gsea, fold_change = gsea1$fc, show_category = 20) +
  rhapsodykit::enrich_cnetplot(gsea2$gsea, fold_change = gsea2$fc, show_category = 20)
```
