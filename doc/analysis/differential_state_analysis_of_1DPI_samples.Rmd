...
```{r}
fromListToUpSetData <- function(input) {
...
# 提取出差异基因列表，指定比较名，返回以 cluster 为组件的列表
diff_state_genelist <- function(results, contrast) {
  results$table %>%
    .[[contrast]] %>%
    purrr::map("gene") %>%
    purrr::compact()
...
gene_sets_list <- ds_res$DESeq2 %>%
  rhapsodykit::diff_state_significant() %>%
  diff_state_genelist("X1DPI.PNR2-X1DPI.MOCK")

make_upset_data <- function(binary) {
  binary %>%
    fromListToUpSetData() %>%
...
upset_plot <- function(df) {
  main_plot <- df %>%
    ...
    ggupset::scale_x_upset(n_intersections = 15, expand = ggplot2::expansion(c(0, 0), c(0.8, 0.8))) +
    ggplot2::scale_y_continuous(expand = ggplot2::expansion(c(0, 0), c(0, 40))) +
    ggplot2::theme_classic() +
    ggplot2::theme(plot.margin = ggplot2::margin(0.5, 0.5, 0.5, 1.5, unit = "cm"))
...
upset_updown_regulated <- function(up_list, down_list) {
  up_data <- make_upset_data(up_list)
  down_data <- make_upset_data(down_list)
...
p1 <- up_list %>%
  make_upset_data() %>%
  upset_plot() + ggplot2::ggtitle("Up-regulated genes")

p2 <- down_list %>%
  make_upset_data() %>%
  upset_plot() + ggplot2::ggtitle("Down-regulated genes")

p1 + p2

upset_updown_regulated(up_list, down_list)
```
