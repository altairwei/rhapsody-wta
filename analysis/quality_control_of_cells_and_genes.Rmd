...
```{r}
count_thr <- attr(
  scater::isOutlier(meta_df$nCount_RNA, log = TRUE, type = "lower"), "thresholds")[1]
feat_thr <- attr(
  scater::isOutlier(meta_df$nFeature_RNA, log = TRUE, type = "lower"), "thresholds")[1]
```
...
ggplot2::ggplot(meta_df, ggplot2::aes(nCount_RNA, fill = group)) +
  ...
  ggplot2::geom_vline(xintercept = count_thr) +
  ggplot2::scale_x_log10() +
ggplot2::ggplot(meta_df, ggplot2::aes(nFeature_RNA, fill = group)) +
  ggplot2::geom_histogram(bins = 100) +
  ggplot2::geom_vline(xintercept = feat_thr) +
  ggplot2::scale_x_log10() +
...
meta_df %>%
  dplyr::mutate(
    isLowerOutlier = scuttle::isOutlier(nCount_RNA, log = TRUE, type = "lower", batch = sample),
    isHigherOutlier = scuttle::isOutlier(nCount_RNA, type = "higher", nmads = 10, batch = sample),
    isOutlier = isLowerOutlier | isHigherOutlier
  ) %>%
  ggplot2::ggplot(ggplot2::aes(x = nCount_RNA, fill = isOutlier)) +
...
meta_df %>%
  dplyr::mutate(
    isLowerOutlier = scuttle::isOutlier(nFeature_RNA, log = TRUE, type = "lower", batch = sample),
...
```{r}
obj_list <- lapply(obj_list, function(obj) {
  obj$isCountOutlier <- scuttle::isOutlier(
    obj$nCount_RNA, log = TRUE, type = "lower")
  obj$isFeatureOutlier <- scuttle::isOutlier(
    obj$nFeature_RNA, log = TRUE, type = "lower")
  obj
})
```
...
* 针对不同批次的样本应用离群值计算来移除不合格的细胞。
  * 超过中位数3倍MAD的值就是离群值。
  * `nCount_RNA` 和 `nFeature_RNA`。
* 叶绿体基因 counts 占比小于 10%，不管线粒体。
...
obj_list <- lapply(obj_list, function(obj) {
  xfun::cache_rds(
    ...
    expr = local({
      subset(
        obj,
        subset = isCountOutlier == FALSE
          & isFeatureOutlier == FALSE
          & percent.mt < 10
          & log10GenesPerUMI > 0.8,
        features = which(
