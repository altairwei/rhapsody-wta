...
plot_bootstrap_distribution <- function(res, clusters = NULL, ncol = NULL, nrow = NULL) {
  # Use BCa as CI
  df_to_plot <- res$results %>%
    tibble::as_tibble() %>%
    dplyr::filter(method == "BCa") %>%
  ...
  ggplot2::ggplot(df_to_plot,
    ...
    ggplot2::geom_line(
      ...
      position = ggplot2::position_dodge(width = 0.4)) +
    ggplot2::scale_y_continuous(labels = scales::percent) +
    ggplot2::facet_wrap(~cellTypes, scales = "free_y", ncol = ncol, nrow = nrow) +
    ggplot2::theme_bw() +
    NULL
}
...
```{r}
plot_mds_by_cluster <- function(df, clusters = NULL, nrow = NULL) {
  if (!is.null(clusters)) {
...
plot_bootstrap_distribution(ca_res_nonclust,
  clusters = c("MCs_1", "ECs_3", "MCs_2", "VCs_1"), nrow = 4) +
...
plot_bootstrap_distribution(ca_res_nonclust,
  clusters = c("MCs_4", "CCs_6", "VCs_5"), nrow = 3) +
plot_mds_by_cluster(pb_mds_df,
  clusters = c("MCs_4", "CCs_6", "VCs_5"), nrow = 3)
```
...
```{r fig.height=4.5, fig.width=6}
plot_bootstrap_distribution(ca_res_nonclust,
  clusters = c("VCs_2", "VCs_3", "VCs_5"), nrow = 3) +
plot_mds_by_cluster(pb_mds_df,
  clusters = c("VCs_2", "VCs_3", "VCs_5"), nrow = 3)
```
...
```{r fig.height=4.5, fig.width=6}
plot_bootstrap_distribution(ca_res_nonclust,
  clusters = c("CCs_2", "CCs_5", "VCs_6"), nrow = 3) +
plot_mds_by_cluster(pb_mds_df,
  clusters = c("CCs_2", "CCs_5", "VCs_6"), nrow = 3)
```
...
plot_bootstrap_distribution(ca_res_nonclust,
  clusters = c("ECs_2", "CCs_5", "VCs_2", "VCs_3", "VCs_5"), nrow = 5) +
plot_mds_by_cluster(pb_mds_df,
  clusters = c("ECs_2", "CCs_5", "VCs_2", "VCs_3", "VCs_5"), nrow = 5)
```
...
```{r fig.height=4.5, fig.width=6}
plot_bootstrap_distribution(ca_res_nonclust,
  clusters = c("MCs_2", "Un_1", "VCs_6"), nrow = 3) +
plot_mds_by_cluster(pb_mds_df,
  clusters = c("MCs_2", "Un_1", "VCs_6"), nrow = 3)
```
...
plot_bootstrap_distribution(ca_res_nonclust,
  clusters = c("CCs_1", "CCs_4", "MCs_3", "Un_2", "MCs_5", "VCs_4"), nrow = 6) +
plot_mds_by_cluster(pb_mds_df,
  clusters = c("CCs_1", "CCs_4", "MCs_3", "Un_2", "MCs_5", "VCs_4"), nrow = 6)
```