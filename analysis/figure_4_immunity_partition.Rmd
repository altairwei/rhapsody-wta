...
```{r}
deg_bool_df <- dplyr::rename(deg_bool_df,
  `Stomata (ST)` = Stomata,
  `Epidermis (EP)` = Epidermis,
  `Chlorenchyma (CH)` = Chlorenchyma,
  `Parenchyma (PA)` = Parenchyma,
  `Outer sheath (VOS)` = `Outer sheath`,
  `Inner sheath (VIS)` = `Inner sheath`,
  `Phloem (PH)` = Phloem,
  `Procambium (PR)` = Procambium
)
```

```{r}
set.seed(20221016)
fit <- eulerr::euler(deg_bool_df, by = list(Treatment, Time), shape = "ellipse")
```
...
eulerr::eulerr_options(
  strips = list(fontsize = 20, font = 1, fontfamily = "Arial"),
  quantities = list(fontsize = 16, fontfamily = "Arial"),
  legend = list(fontsize = 18, vgap = grid::unit(0.8, "lines"),
                fontfamily = "Arial"))
...
```{r eval=FALSE, include=FALSE}
plotpowerpoint(p6, "../data/Figure.pptx",
  location = grid::unit(c(3.29, 39.68, 47.28, 20), "cm"),
  fonts = list(sans = "Arial", serif = "Arial", mono = "Courier"))
```
...
```{r}
deg_bool_df_simplified <- deg_bool_df[, c("Parenchyma (PA)", "Outer sheath (VOS)", "Phloem (PH)", "Time", "Treatment")]
```

```{r}
set.seed(20221016)
fit <- eulerr::euler(deg_bool_df_simplified, by = list(Treatment, Time), shape = "ellipse")
```
...
p6 <- plot(
  ...
  labels = FALSE,
  quantities = TRUE,
  fills = all_tissues_colors[c("Parenchyma", "Outer sheath", "Phloem")]
)
...
plot_updown <- function(ds, contrast) {
  ...
  up_data <- rhapsodykit::from_list_to_upset(up_list) %>% rhapsodykit::upset_dataframe()
  down_data <- rhapsodykit::from_list_to_upset(down_list) %>% rhapsodykit::upset_dataframe()

  up_data$regulated <- factor("Up-regulated", c("Up-regulated", "Down-regulated"))
  down_data$regulated <- factor("Down-regulated", c("Up-regulated", "Down-regulated"))
  ...
  df_to_plot %>%
    ...
    ggplot2::labs(x = "", y = "Count", fill = "") +
    ggplot2::geom_text(
      stat = "count", size = 14 * FT, vjust = -1,
      position = ggplot2::position_dodge(width = 0.9),
      mapping = ggplot2::aes(label = ggplot2::after_stat(count))) +
    ggupset::scale_x_upset(
      expand = ggplot2::expansion(c(0, 0), c(0.8, 0.8)),
      n_intersections = 15) +
    ggplot2::scale_y_continuous(expand =  ggplot2::expansion(c(0, 0.3), c(0, 0))) +
    ggplot2::theme_classic(base_size = 16) +
    ggupset::theme_combmatrix(
      combmatrix.label.make_space = FALSE,
      combmatrix.label.text = ggplot2::element_text(size = 18),
      combmatrix.panel.point.size = 5,
      plot.margin = ggplot2::margin(0.5, 0.5, 0.5, 1.5, unit = "cm"),
      axis.line = ggplot2::element_line(linewidth = 1.5 * PT),
...
plist <- lapply(
  X = c(
    ...
    "X3DPI.TR4-X3DPI.MOCK"),
  FUN = function(cont) {
    plot_updown(ds_sig, cont) +
      ggplot2::ggtitle(cont)
  }
)
...
```{r}
sce$cluster.idnum <- factor(
  sce$RNA_snn_res.0.6, levels = all_cluster_num[all_celltypes])

sce$predicted.idnum <- factor(
  mock_cluster_num[as.character(sce$predicted.id)],
  levels = mock_cluster_num[mock_celltypes])
```

```{r fig.height=7, fig.width=14}
p1 <- scater::plotReducedDim(sce, dimred = "UMAP",
                             colour_by = "predicted.idnum",
                             text_by = "predicted.idnum",
                             text_size = 16 * FT,
                             point_size = 0.5,
                             theme_size = 22) +
  ggplot2::scale_color_manual(
    values = setNames(mock_celltype_colors, mock_cluster_num[mock_celltypes])) +
  ggplot2::ggtitle("Predicted Cluster") +
...
p2 <- scater::plotReducedDim(sce, dimred = "UMAP",
  ...
  ggplot2::ggtitle("Unsupervised Cluster") +
  legend_override("color", list(size = 4, alpha = 1)) +
  ggplot2::coord_fixed() +
  theme_dimred(linesize = 1.5 * PT) +
  ggplot2::theme(legend.title = ggplot2::element_blank(),
...
p2 <- ggrastr::rasterise(p2)

p1 + p2
```

### Alluvial Plot of Cell Labels

```{r}
df <- as.data.frame(
  colData(sce)[, c("predicted.idnum", "cluster.idnum",
                   "tissue", "body_layer", "prediction.score.max")])
```

```{r}
df_sankey <- df |>
  dplyr::group_by(predicted.idnum, cluster.idnum, tissue, body_layer) |>
...
df_sankey$predicted.idnum <- forcats::fct_relabel(
  df_sankey$predicted.idnum, ~ paste0("C", .x))
```
...
p3 <- ggplot2::ggplot(
  data = df_sankey,
  mapping = ggplot2::aes(
    axis1 = predicted.idnum,
    axis2 = cluster.idnum,
    axis3 = tissue,
    axis4 = body_layer,
    y = freq)) +
  ggalluvial::geom_alluvium(
    mapping = ggplot2::aes(fill = cluster.idnum)) +
  ...
  ggplot2::geom_text(
    ...
    mapping = ggplot2::aes(label = forcats::fct_relabel(
      ggplot2::after_stat(stratum), ~ stringr::str_remove(.x, "^C(?=\\d.*)"))),
    size = 16 * FT) +
  ggplot2::scale_y_continuous(expand = c(0, 0.02)) +
  ggplot2::scale_x_continuous(position = "top", expand = c(0, 0.005),
                              labels = c("Predicted", "Unsupervised", "Cell Type", "Body Layer")) +
  ggplot2::theme_void() +
  remove_legend() +
  ggplot2::theme(axis.text.x.top = ggplot2::element_text(size = 20))
...
```{r}
p4 <- ggplot2::ggplot(df, ggplot2::aes(
    x = cluster.idnum, y = prediction.score.max, fill = tissue)) +
  #ggplot2::geom_boxplot(outlier.size = .5) +
  ggplot2::geom_violin(scale = "width") +
  ggplot2::scale_fill_manual(
    values = all_tissues_colors) +
  ggplot2::labs(x = "Unsupervised Cluster", y = "Prediction Score", fill = "Cell Type") +
  ggplot2::theme_bw(22) +
  ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 45, hjust = 1),
                 legend.box.spacing = grid::unit(1, "mm"),
                 panel.border = ggplot2::element_rect(size = 1.5 * PT))
...
```{r}
ca_res_nonclust_8ct <- xfun::cache_rds(
  file = "ca_res_nonclust_8celltypes.rds",
  ...
  expr = local({
    ...
    res$results$cond <- obj$group[match(res$results$subject, obj$sample)]
    res$info$cond <- obj$group[match(res$info$subject, obj$sample)]
...
```{r}
ca_res_nonclust_8ct$results$cellTypes <- forcats::fct_relevel(ca_res_nonclust_8ct$results$cellTypes, TISSUE_TYPES)
```

```{r}
p5 <- local({
  df_to_plot <- ca_res_nonclust_8ct$results %>%
    tibble::as_tibble() %>%
  ...
  prop_df <- ca_res_nonclust_8ct$thetastar %>%
    as.data.frame()
  
  colnames(prop_df) <- paste("BS_", seq_len(ncol(ca_res_nonclust_8ct$thetastar)), sep = "")
  ...
  ggplot2::ggplot(df_to_plot, ggplot2::aes(x = time, y = prop, color = treatment)) +
    ggplot2::geom_boxplot(
      width = 0.2, outlier.shape = NA, linewidth = 1.5 * PT,
      position = ggplot2::position_dodge(width = 0.4)) +
    ggplot2::geom_line(
      mapping = ggplot2::aes(group = treatment),
      stat = "summary", fun = median, linewidth = 1.5 * PT,
      position = ggplot2::position_dodge(width = 0.4)) +
    ggplot2::scale_y_continuous(labels = scales::percent) +
    ggplot2::facet_wrap(~ cellTypes, scales = "free_y", nrow = 2, drop = FALSE) +
    ggplot2::scale_color_brewer(type = "qual", palette = "Dark2") +
    ggplot2::guides(fill = "none") +
    ggplot2::labs(x = "Time", y = "Proportion", color = "Treatment") +
    ggplot2::theme_bw(base_size = 22) +
    ggplot2::theme(
      panel.grid = ggplot2::element_blank(),
...
p <- (p1 + p2) / p3 / p4 / p5 +
  patchwork::plot_layout(heights = c(0.8, 1.2, 0.4, 0.6)) &
  ggplot2::theme(legend.box.spacing = grid::unit(1, "mm"))
...
```{r eval=FALSE, include=FALSE}
plotpowerpoint(p, "../data/Figure.pptx",
  location = grid::unit(c(2.5, 4.53, 49.01, 63), "cm"),
  fonts = list(sans = "Arial", serif = "Arial", mono = "Courier"))
```
...
```{r}
set.seed(20221016)
fit_os <- eulerr::euler(deg_glist_os, shape = "ellipse")

set.seed(20221016)
fit_ph <- eulerr::euler(deg_glist_ph, shape = "ellipse")
```
...
```{r}
p12fun <- function() {
  gridExtra::grid.arrange(gridExtra::arrangeGrob(grobs = list(p1, p2), ncol = 2))
}
```

```{r eval=FALSE, include=FALSE}
plotpowerpoint(p1, "../data/Figure.pptx", fun = p12fun,
  location = grid::unit(c(2.5, 4.49, 23.96, 19.77), "cm"),
  fonts = list(sans = "Arial", serif = "Arial", mono = "Courier"))
```

#### Compare TR4 and PNR2
...
deg_bool_df <- local({
  boolmtx <- ds_sig %>%
    rhapsodykit::diff_state_subset(
      contrast = c(
        ...
        "X3DPI.PNR2-X3DPI.MOCK",
        "X3DPI.TR4-X3DPI.MOCK"),
      clusters = c("Outer sheath", "Phloem")) %>%
    upset_contrasts_list() |>
    rhapsodykit::from_list_to_upset()
  ...
  df <- boolmtx |>
    as.data.frame() |>
    tibble::rownames_to_column("Gene") |>
    tidyr::pivot_longer(!Gene, names_to = c("Group", "CellType"),
                        names_pattern = "(\\dDPI-.*) \\[(.*)\\]",
                        values_to = "Logical") |>
    tidyr::pivot_wider(names_from = Group, values_from = Logical,
                       values_fill = FALSE) |>
    dplyr::select(CellType, "1DPI-PNR2", "2DPI-PNR2", "3DPI-PNR2", "1DPI-TR4", "2DPI-TR4", "3DPI-TR4")
...
```{r}
set.seed(20221016)
fit <- eulerr::euler(deg_bool_df, by = CellType, shape = "ellipse")
```
...
deg_bool_df <- local({
  ...
  df <- boolmtx |>
    ...
    tidyr::pivot_longer(!Gene, names_to = c("Time", "Treatment", "CellType"),
                        names_pattern = "(\\dDPI)-(.*) \\[(.*)\\]",
                        values_to = "Logical") |>
    tidyr::pivot_wider(names_from = Time, values_from = Logical,
                       values_fill = FALSE) |>
    dplyr::select(!Gene)
...
```{r}
set.seed(20221016)
fit <- eulerr::euler(deg_bool_df, by = list(Treatment, CellType), shape = "ellipse")
```
...
### Enrichment Analysis of 1 DPI

```{r}
ora_comp_1dpi <- readRDS(Sys.glob(
  "../results/ObjectCache/DifferentialState/ds_ora_comp_1dpi_*.rds"))
```
...
p1 <- ora_comp_1dpi$BP |>
  dplyr::mutate(
    regu = stringr::str_extract(Cluster, "induced|suppressed"),
  ...
  ggplot2::facet_grid(regu ~ cluster_id, scales = "free", space = "free") +
  #rotate_x_labels() +
  ggplot2::theme(
...
plotpowerpoint(p1, "../data/Figure.pptx",
  location = grid::unit(c(27, 5.12, 24.5, 19.14), "cm"),
  fonts = list(sans = "Arial", serif = "Arial", mono = "Courier"))
```

### Enrichment Analysis of 2 DPI

```{r}
ora_comp_2dpi <- readRDS(Sys.glob(
  "../results/ObjectCache/DifferentialState/ds_ora_comp_2dpi_*.rds"))
```
...
plotpowerpoint(p2, "../data/Figure.pptx",
  location = grid::unit(c(2.5, 24.97, 49.01, 19.14), "cm"),
  fonts = list(sans = "Arial", serif = "Arial", mono = "Courier"))
```

### Enrichment Analysis of 3 DPI

```{r}
ora_comp_3dpi <- readRDS(Sys.glob(
  "../results/ObjectCache/DifferentialState/ds_ora_comp_3dpi_*.rds"))
```
...
```{r eval=FALSE, include=FALSE}
plotpowerpoint(p3, "../data/Figure.pptx",
  location = grid::unit(c(2.5, 44.83, 49.01, 19.14), "cm"),
  fonts = list(sans = "Arial", serif = "Arial", mono = "Courier"))
```

## Supplementary Figure 5X

```{r}
Phloem_cols <- c(
  "Phloem.PNR2-induced" = "#4e79a7",
  "Phloem.PNR2-suppressed" = "#f28e2b",
  "Phloem.TR4-induced" = "#e15759",
  "Phloem.TR4-suppressed" = "#76b7b2"
)

Outersheath_cols <- c(
  "Outer sheath.PNR2-induced" = "#4e79a7",
  "Outer sheath.PNR2-suppressed" = "#f28e2b",
  "Outer sheath.TR4-induced" = "#e15759",
  "Outer sheath.TR4-suppressed" = "#76b7b2"
)
```


### Enrichment Map of 1 DPI
...
set.seed(1124)
p1 <- ora_comp_1dpi$BP |>
  dplyr::filter(cluster_id == "Phloem") |>
  ...
  ggplot2::scale_fill_manual(values = Phloem_cols) +
  ggplot2::ggtitle("Enrichment Map of 1 DPI") +
...
```{r eval=FALSE, include=FALSE}
plotpowerpoint(p2, "../data/Figure.pptx",
  location = grid::unit(c(2.5, 4.49, 19.77, 19.77), "cm"),
  fonts = list(sans = "Arial", serif = "Arial", mono = "Courier"))
```
...
set.seed(1124)
p3 <- ora_comp_1dpi$BP |>
  dplyr::filter(cluster_id == "Outer sheath") |>
...
plotpowerpoint(p4, "../data/Figure.pptx",
  location = grid::unit(c(24.27, 4.49, 19.77, 19.77), "cm"),
  fonts = list(sans = "Arial", serif = "Arial", mono = "Courier"))
```

### Enrichment Map of 2 DPI
...
p1 <- ora_comp_2dpi$BP |>
  ...
  ggplot2::guides(fill = ggplot2::guide_legend(title = "DEG Sets")) +
  ggthemes::scale_fill_tableau(palette = "Tableau 10") +
  ggplot2::ggtitle("Enrichment Map of 2 DPI") +
  ggplot2::theme_void(14) +
  center_plot_title()
...
```{r eval=FALSE, include=FALSE}
plotpowerpoint(p2, "../data/Figure.pptx",
  location = grid::unit(c(2.5, 29.26, 19.77, 19.77), "cm"),
  fonts = list(sans = "Arial", serif = "Arial", mono = "Courier"))
```
...
```{r eval=FALSE, include=FALSE}
plotpowerpoint(p4, "../data/Figure.pptx",
  location = grid::unit(c(24.27, 29.26, 19.77, 19.77), "cm"),
  fonts = list(sans = "Arial", serif = "Arial", mono = "Courier"))
```
...
p1 <- ora_comp_3dpi$BP |>
  ...
  ggplot2::guides(fill = ggplot2::guide_legend(title = "DEG Sets")) +
  ggthemes::scale_fill_tableau(palette = "Tableau 10") +
  ggplot2::ggtitle("Enrichment Map of 3 DPI") +
  ggplot2::theme_void(14) +
  center_plot_title()
...
```{r eval=FALSE, include=FALSE}
plotpowerpoint(p2, "../data/Figure.pptx",
  location = grid::unit(c(2.5, 54.03, 19.77, 19.77), "cm"),
  fonts = list(sans = "Arial", serif = "Arial", mono = "Courier"))
```
...
```{r eval=FALSE, include=FALSE}
plotpowerpoint(p4, "../data/Figure.pptx",
  location = grid::unit(c(24.27, 54.03, 19.77, 19.77), "cm"),
  fonts = list(sans = "Arial", serif = "Arial", mono = "Courier"))
```
...
```{r}
df_immune_genes <- readxl::read_excel("../data/Supplementary Table 4 - A curated list of immunity related genes.xlsx")
...
```{r}
df_immune_gsva <- local({
  df_immune_genes_filtered <- df_immune_genes |>
    dplyr::filter(`Category in brief` %in% c(
      "TaRLK", "TaNLR", "RBOH", "PRNPR", "CaSignaling",
      "MAPK", "Benzox", "NHPbiosyn", "PALrelated", "TaGST"))
  
  expr_gsva <- GSVA::gsva(
    pseudobulk_cpm_mean, list(set1=df_immune_genes_filtered$gene),
  ...
  gsva_df$cellType <- factor(gsva_df$cellType, levels = TISSUE_TYPES)
...
plotRadarGSVA(df_immune_gsva, "set1")
```
...
df_deg <- df_deg |>
  dplyr::group_by(gene) |>
  dplyr::mutate(hits = dplyr::n(), regu_score = sum(logFC / abs(logFC))) |>
  dplyr::ungroup() |>
  dplyr::select(
    gene, description, hits, regu_score,
    cluster_id, time, comparison, contrast,
    dplyr::everything())
```
...
```{r}
dplyr::sample_n(df_deg, 20)
```

```{r eval=FALSE}
openxlsx::write.xlsx(df_deg, "../results/DataPlots/SupplementaryTable_2_DEGs.xlsx")
```
...
```{r}
ORA_dfs <- lapply(
  list("1DPI" = ora_comp_1dpi$BP,
       "2DPI" = ora_comp_2dpi$BP,
       "3DPI" = ora_comp_3dpi$BP),
  function(BP) {
    p <- BP |>
      enrichplot::pairwise_termsim(
        showCategory = NULL) |>
    ...
    as.data.frame(BP) |>
      dplyr::arrange(dplyr::desc(match(Description, paths))) |>
      dplyr::select(Description, ID, dplyr::everything())
  })
```
...
for (sheet in names(ORA_dfs)) {
  openxlsx::addWorksheet(wb, sheet)
  openxlsx::writeDataTable(wb, sheet, ORA_dfs[[sheet]])
}

openxlsx::saveWorkbook(
  wb, file = "../results/DataPlots/SupplementaryTable_3_DS_ORA.xlsx",
  overwrite = TRUE)
```
