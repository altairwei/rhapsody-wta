...
obj_mock <- Seurat::FindNeighbors(obj_mock, reduction = "harmony")
...
Seurat::DimPlot(obj_mock, reduction = "tsne", split.by = "time") &
  ggplot2::coord_fixed()
```

### Association results {.tabset}
...
```{r}
obj_pnr2 <- subset(obj, subset = group %in% c("0DPI-MOCK", "1DPI-PNR2", "2DPI-PNR2", "3DPI-PNR2"))
obj_pnr2$time_val <- as.numeric(factor(obj_pnr2$time, levels = c("0DPI", "1DPI", "2DPI", "3DPI")))
obj_pnr2 <- Seurat::FindNeighbors(obj_pnr2, reduction = "harmony")

obj_pnr2 <- rcna::association.Seurat(
  seurat_object = obj_pnr2,
  test_var = "time_val",
  samplem_key = "sample",
  graph_use = "RNA_nn",
  batches = NULL,
...
```{r fig.height=8, fig.width=16}
p1 <- Seurat::FeaturePlot(obj_pnr2, reduction = "tsne", features = c("cna_ncorrs")) &
  ggplot2::scale_color_gradientn(colours = blue_to_orange) &
  ggplot2::coord_fixed()

p2 <- Seurat::DimPlot(obj_pnr2, reduction = "tsne", group = "group") &
  ggplot2::coord_fixed()
...
```{r fig.height=8, fig.width=16}
p1 <- Seurat::FeaturePlot(obj_pnr2, reduction = "umap", features = c("cna_ncorrs")) &
  ggplot2::scale_color_gradientn(colours = blue_to_orange) &
  ggplot2::coord_fixed()

p2 <- Seurat::DimPlot(obj_pnr2, reduction = "umap", group = "group") &
  ggplot2::coord_fixed()
...
```{r}
obj_tr4 <- subset(obj, subset = group %in% c("0DPI-MOCK", "1DPI-TR4", "2DPI-TR4", "3DPI-TR4"))
obj_tr4$time_val <- as.numeric(factor(obj_tr4$time, levels = c("0DPI", "1DPI", "2DPI", "3DPI")))
obj_tr4 <- Seurat::FindNeighbors(obj_tr4, reduction = "harmony")

obj_tr4 <- rcna::association.Seurat(
  seurat_object = obj_tr4,
  test_var = "time_val",
  samplem_key = "sample",
  graph_use = "RNA_nn",
  batches = NULL,
...
```{r fig.height=8, fig.width=16}
p1 <- Seurat::FeaturePlot(obj_tr4, reduction = "tsne", features = c("cna_ncorrs")) &
  ggplot2::scale_color_gradientn(colours = blue_to_orange) &
  ggplot2::coord_fixed()

p2 <- Seurat::DimPlot(obj_tr4, reduction = "tsne", group = "group") &
  ggplot2::coord_fixed()
...
```{r fig.height=8, fig.width=16}
p1 <- Seurat::FeaturePlot(obj_tr4, reduction = "umap", features = c("cna_ncorrs")) &
  ggplot2::scale_color_gradientn(colours = blue_to_orange) &
  ggplot2::coord_fixed()

p2 <- Seurat::DimPlot(obj_tr4, reduction = "umap", group = "group") &
  ggplot2::coord_fixed()
...
```{r factorize}
obj_sub <- Seurat::FindNeighbors(obj_sub, reduction = "harmony")
