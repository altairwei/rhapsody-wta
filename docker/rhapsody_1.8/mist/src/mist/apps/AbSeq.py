...
def calculate_readmolsum(z_pabo, z_mrna, putative_cells, pabo_dir, exp_name):
    total_rna_reads = z_mrna.groupby(z_mrna.index.get_level_values(0))['reads'].apply(sum).sort_values(ascending=False)
    total_rna_rsecmols = z_mrna.groupby(z_mrna.index.get_level_values(0))['rsec_mols'].apply(sum)
    total_pabo_reads = z_pabo.groupby(z_pabo.index.get_level_values(0))['reads'].apply(sum)
    total_pabo_rsecmols = z_pabo.groupby(z_pabo.index.get_level_values(0))['rsec_mols'].apply(sum)
    # get data table of cell by pabo reads
    z_pabo.reset_index(inplace=True)
    z_pabo_unstack = z_pabo.groupby(['cell','gene'])['reads'].apply(sum).unstack(level=1)
    # merge data tables and save
    combined_df = pd.concat([total_rna_reads, total_rna_rsecmols, total_pabo_reads, total_pabo_rsecmols], axis=1)
...
def calculate_totalReadsPerAbo(z_pabo, full_gene_list, pabo_dir, exp_name):
    ...
    actual_reads = z_pabo.groupby(z_pabo.index.get_level_values(1))['reads'].apply(sum)
