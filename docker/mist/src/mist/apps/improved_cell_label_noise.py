...
def main(cell_read_counts, num_cell_image, sample, split_file_list, num_genes_in_panel, output_header, stats_file, assay):
    ...
    for cutoff in z_cutoffs:
        ...
        if len(variable_genes) < 1: 
            ...
            read_counts_var_genes = z_var_genes.groupby(z_var_genes.index.get_level_values(0))['dbec_reads'].apply(sum).sort_values(ascending=False)
            try:
    ...
    read_counts_all_cells = z.groupby(z.index.get_level_values(1))['dbec_reads'].apply(sum)
    # get the read counts from (updated) filtered cells
    read_counts_filtered_cells = z_filtered_cells.groupby(z_filtered_cells.index.get_level_values(1))['dbec_reads'].apply(sum)
    ...
    if len(dropout_genes) < 1:
        ...
        read_counts_dropout_genes = z_dropout_genes.groupby(z_dropout_genes.index.get_level_values(0))['dbec_reads'].apply(sum).sort_values(ascending=False)
        try:
    ...
    tot_mols = z_comb.groupby(z_comb.index.get_level_values(0))['dbec_mols'].apply(sum).sort_values(ascending=False)
    ...
    # sort based on the total reads to be consistent with the order of cells in the ReadsPerCell and MolsPerCell DTs
    sorted_tot_reads = z_final.groupby(z_final.index.get_level_values(0))['reads'].apply(sum).sort_values(ascending=False, kind = 'mergesort')
