...
def find_expressed_genes(dt, len_header):
...
def find_expressed_abos(dt, len_header):
    """Drops the genes not expressed in data tables."""
    df = pd.read_csv(dt, header=len_header)
    not_expressed = []
    df.set_index('Cell_Index', inplace=True)
...
def simple_iter_sam(sam_fp):
    """
    iterate over sam file while skipping header lines and stripping the tags
    TODO: refactor with pysam
    """
    with open(sam_fp, mode='rt') as sam:
        for line in sam:
            if not line.startswith('@'):  # skip header lines
                aligned_segment = line.strip().split('\t')
                qname, flag, gene, pos, mapq, cigar, rnext, pnext, tlen, seq, qual = aligned_segment[:11]
                tags = aligned_segment[11:]
                yield qname, int(flag), gene, int(pos), mapq, cigar, rnext, pnext, tlen, seq, qual, tags
