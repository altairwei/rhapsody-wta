...
def cli():
    des = 'Sparse_to_Dense_Datatable, ' + _v.desc
    parser = argparse.ArgumentParser(description=des, add_help=True)
    parser.add_argument('--sparse-data-table',
                        dest='sparse_dt',
                        help='Sparse data table')
...
@logging.log_death
@utils.node_timer
def sparse_to_dense_conversion(sparse_dt, cell_order, gene_list):
    logging.info("Start exporting dense data table")
    output_header, run_info = utils.grab_main_header(sparse_dt)
    dt_type = 'Unfiltered' if 'Unfiltered' in os.path.basename(sparse_dt) else 'Filtered'
    output_fp = os.path.basename(sparse_dt).replace('_Sparse', '').replace('.gz', '')
    ...
    with open(output_fp, mode='wt') as f_out:
        ...
        df = pd.read_csv(sparse_dt, header=len(output_header), names=[
                        'Cell_Index',
                        'Gene',
    ...
    logging.info("End exporting dense data table")


def main():
    sparse_to_dense_conversion(**cli())
