...
# TODO: most this code could be replaced with simple import statements from umi_tools
# e.g. from umi_tools.network import get_adjacency_list
# !! deleting code is the easiest way to maintain it !!
...
def seq_error_correction(umi_counter, cell_index, gene):
    # type: (Counter, str, str) -> Generator[List[obj]]
    """Ingress for RSEC; yields an annotated molecule, not including DBEC correction

    Args:
        umi_counter (Counter): dictionary that relates the UMI to the count of its reads
        cell_index: cell label; not used, just yielded
        gene: gene; not used, just yielded

    Yields: [cell-label, barcode, gene, read_count, rsec_corrected_read_count, parent_barcode]
        If the barcode is a parent -- that is to say, it is not an error -- then parent_barcode is empty.
    """

    reduced_clusters, child_to_parent_umi = demultiplex_umi(umi_counter)
...
def demultiplex_umi(umi_counter):
    """apply rsec to a distribution of umis

    Args:
        umi_counter: dictionary that relates the UMI to the count of its read

    Returns: (reduced_clusters, child_to_parent_umi) tuple
        reduced_clusters
        child_to_parent_umi: child umi to parent umi dictionary; parent umis not represented
