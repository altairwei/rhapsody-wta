...
```{r}
adata_me_da_1dpi <- cache_pickle(
  file = "unitvelo_me_da_1dpi.pickle",
  dir = "../results/ObjectCache/TrajectoryInference/",
  expr = runUniTVelo(
    sce_me_da_1dpi,
    label = "cluster_id",
...
```{r}
sce_me_da_1dpi <- sce_me_da_1dpi[, -which(sce_me_da_1dpi$slingPseudotime_1 > 45)]
```

```{r}
sce_me_da_1dpi <- slingshot::slingshot(sce_me_da_1dpi,
    clusterLabels = sce_me_da_1dpi$cluster_id,
    reducedDim = "HARMONY", start.clus = "Me_1",
    approx_points = 500)
```
...
```{r eval=FALSE}
res <- runScanorama(sce_me_1)
```

每个样本分别寻找 HVGs，最后找到的交集只有 15 个基因，这产出的矫正后矩阵基本上算是废了。
