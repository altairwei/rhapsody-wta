...
```{r stg-markers, cache=TRUE, eval=FALSE, results="hide"}
STG_markers <- DAseq::STGmarkerFinder(
  X = obj_sub@assays$RNA@data,
  da.regions = da_regions, 
  lambda = 1.5, n.runs = 5, return.model = T, 
  GPU = 1, python.use = Sys.which("python")
)
```
...
plot_cell_score <- function(obj, reduction) {
  embedding <- obj$embeddings[[reduction]][obj$cells$cell.idx,]
  DAseq::plotCellScore(X = embedding, score = obj$cells$da.pred) +
    ggplot2::theme(legend.title = ggplot2::element_blank())
}
...
```{r run-pnr2-and-tr4, cache=TRUE, dependson=c("combine-steps", "read-seurat-obj")}
PNR2_da <- find_diff_abundant_cells(obj,
  stim = paste(1:3, "DPI-PNR2", sep = ""),
  ctrl = paste(0:3, "DPI-MOCK", sep = "")
)

TR4_da <- find_diff_abundant_cells(obj,
...
```{r run-pnr2-vs-tr4, cache=TRUE, dependson=c("combine-steps", "read-seurat-obj")}
PNR2_vs_TR4_da <- find_diff_abundant_cells(obj,
  stim = paste(1:3, "DPI-PNR2", sep = ""),
...
patchwork::wrap_plots(
  ...
  plot_da_site(PNR2_vs_TR4_da, "tsne") +
    ggplot2::ggtitle("PNR2 vs. TR4")
)
```
...
patchwork::wrap_plots(
  plot_cell_score(PNR2_vs_TR4_da, "umap") +
    ggplot2::ggtitle("PNR2 vs. TR4"),
  plot_da_site(PNR2_vs_TR4_da, "umap") +
    ggplot2::ggtitle("PNR2 vs. TR4")
)
```
...
```{r 1dpi-sample-data, cache=TRUE, dependson=c("combine-steps", "read-seurat-obj"), warning=FALSE, message=FALSE, results="hide"}
results_1dpi <- lapply(
  list(
...
generate_da_plots <- function(results) {
  for (reduc in c("tsne", "umap")) {
    ...
    cat("\n\n")
    print(plot_da_overlap(results[[1]]) / plot_da_overlap(results[[2]]))
    cat("\n\n")
  }
}
...
```{r 2dpi-sample-data, cache=TRUE, dependson=c("combine-steps", "read-seurat-obj"), warning=FALSE, message=FALSE, results="hide"}
results_2dpi <- lapply(
  list(
...
```{r 3dpi-sample-data, cache=TRUE, dependson=c("combine-steps", "read-seurat-obj"), warning=FALSE, message=FALSE, results="hide"}
results_3dpi <- lapply(
  list(
