...
Seurat::DimPlot(obj, reduction = "tsne", group.by = "time", split.by = "treatment") +
...
```{r pb-mds-plot}
pb_mds_df <- xfun::cache_rds(
  file = "pb_mds_df.rds",
  dir = "../results/ObjectCache/DifferentialState/",
  expr = local({
    data <- muscat::pbMDS(pb_list$logcounts)$data
    data$time <- rhapsodykit::key_mapping(data$group_id, c(
    ...
    data$treatment <- rhapsodykit::key_mapping(data$group_id, c(
      "X0DPI.MOCK" = "MOCK",
      "X1DPI.MOCK" = "MOCK",
      "X1DPI.PNR2" = "PNR2",
      "X1DPI.TR4"  = "TR4",
...
```{r mds-plot-by-time, fig.height=7, fig.width=10}
group_colors <- c(
  "X0DPI.MOCK" = "#3DD8FF",
  "X1DPI.MOCK" = "#75B0BF",
  "X2DPI.MOCK" = "#2B97B3",
...
ds_res$DESeq2 %>%
  rhapsodykit::diff_state_significant() %>%
  rhapsodykit::diff_state_pull("X1DPI.PNR2-X1DPI.MOCK", 1:13, "gene") %>%
...
ds_res$DESeq2 %>%
  rhapsodykit::diff_state_significant() %>%
  rhapsodykit::diff_state_pull("X1DPI.TR4-X1DPI.MOCK", 1:13, "gene") %>%
  rhapsodykit::from_list_to_upset() %>%
  rhapsodykit::upset_dataframe() %>%
  rhapsodykit::upset_plot()
```
...
plot_updown <- function(contrast) {
  up_list <- ds_res$DESeq2 %>%
    ...
    rhapsodykit::diff_state_significant() %>%
    rhapsodykit::diff_state_pull(contrast, 1:13, "gene")
...
ds_res$DESeq2 %>%
  rhapsodykit::diff_state_significant() %>%
  diff_state_subset(cluster = "Va_4") %>%
  upset_contrasts()
```
...
scater::plotExpression(sce[, sce$cluster_id == "Va_4"],
  features = tbl_filtered$`Va_4`$gene[seq_len(6)],
...
ds_res_time <- xfun::cache_rds(
  file = "ds_timeline.rds",
  dir = "../output/",
  expr = {
    list(
      edgeR = rhapsodykit::pseudobulk_diff_state(
