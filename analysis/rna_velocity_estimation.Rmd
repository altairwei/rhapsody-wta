...
```{python fig.height=4, fig.width=12}
df = r.adata.var
...
scv.pl.scatter(r.adata, basis="tsne", color='latent_time', color_map='gnuplot', size=20)
```

##### UMAP

```{python fig.height=7, fig.width=7}
scv.pl.scatter(r.adata, basis="umap", color='latent_time', color_map='gnuplot', size=20)
```

#### Top-likelihood genes

```{python fig.height=12, fig.width=12}
top_genes = r.adata.var['fit_likelihood'].sort_values(ascending=False).index
scv.pl.scatter(r.adata, basis=top_genes[:15], color="cellType", ncols=5, frameon=False)
```

#### Identify important genes

```{python}
scv.tl.rank_velocity_genes(r.adata, groupby='cellType', min_corr=.3)
df = scv.DataFrame(r.adata.uns['rank_velocity_genes']['names'])
```

```{r}
head(reticulate::py$df)
```

```{python fig.height=4, fig.width=12}
kwargs = dict(frameon=False, size=10, linewidth=1.5, color="cellType")

scv.pl.scatter(r.adata, df['CC'][:5], ylabel='CC', **kwargs)
scv.pl.scatter(r.adata, df['Me_1'][:5], ylabel='Me_1', **kwargs)
```

#### Speed and coherence {.tabset}
...
scv.pl.scatter(r.adata, c=keys, cmap='coolwarm', basis="tsne", perc=[5, 95])
```
...
scv.tl.velocity_confidence(r.adata)
keys = 'velocity_length', 'velocity_confidence'
scv.pl.scatter(r.adata, c=keys, cmap='coolwarm', basis="umap", perc=[5, 95])
```

#### Velocity graph and pseudotime {.tabset}

##### t-SNE

```{python fig.height=7, fig.width=7}
scv.pl.velocity_graph(r.adata, threshold=0.8, basis="tsne", color="cellType")
```

##### UMAP

```{python fig.height=7, fig.width=7}
scv.pl.velocity_graph(r.adata, threshold=.8, basis="umap", color="cellType")
```

#### PAGA velocity graph {.tabset}
...
> In dynamo, we also implemented the traditional RNA velocity method based on the steady state assumptions to support analyzing regular 10X data.

从 dynamo 自带的样本数据来看，它对 AnnData 的格式需求仅仅是拥有 spliced 和 unspliced 数据层而已。

> First / second moments are basically mean and uncentered variance of gene expression, which are calculated based on local smoothing via a nearest neighbours graph, constructed in the reduced PCA space from the spliced or total mRNA expression of single cells.

> Kinetic estimation of the conventional scRNA-seq and metabolic labeling based scRNA-seq is often tricky and has a lot pitfalls. Sometimes you may even observed undesired backward vector flow. You can evaluate the confidence of gene-wise velocity and filter those low confidence genes for downstream Velocity vectors analysis.

在计算 moments 时涉及到 nearest neighbours graph, 所以我们要想办法给 dynamo 提供整合后的 harmony 细胞嵌入。

### 4.1 读取数据

dynamo 模块的导入很成问题，首先它会清除当前 python 环境，然后第一次导入还不成功，要导入第二次。所以我们直接在 R 环境中直接通过 reticulate 来使用 dynamo
...
adata <- runDynamoVelocity(sce_mock,
  use.dimred = "HARMONY",
  assay.X = "counts",
  mode = "negbin",
  n_top_genes = 10000L,
  py.params = list(
    filter_and_normalize = list(
      min_shared_counts = 20L, n_top_genes = 2000L),
    dynamics = list(use_smoothed = FALSE)
  ))
```
...
```{r}
dyn$tl$gene_wise_confidence(adata, group='cellType')
```


```{r}
dyn$tl$cell_velocities(adata, method='pearson', other_kernels_dict=list(transform = 'sqrt'))
```
...
dyn$pl$streamline_plot(
  adata, color = c("cellType"), basis = "tsne",
  show_legend = "on data",
...
```{r fig.height=7, fig.width=7}
dyn$pl$streamline_plot(
  adata, color = c("cellType"), basis = "umap",
  show_legend = "on data",
  show_arrowed_spines = FALSE)
```
...
dyn$pl$cell_wise_vectors(
  adata, color = 'cellType', basis = 'umap',
...
UniTVelo 接口设计不好，没有接受 cell embedding 的地方。除非构建一个简化的 anndata，将 X_harmony 命名成 X_pca

```{python}
adata = utv.run_model(adata, label="cellType", config_file=velo)
```

CPU 模式的运行时间太长了！完全不实用。

```{python}
scv.pl.velocity_embedding_stream(adata, basis="tsne", color="cellType", figsize=(7, 7), show=True)
```
