...
def assign_singletons(X,y,preprocessing=log_correlation,verbose=False,
    ...
    sing_to_orig_map = np.array(list(range(len(y))))[sing_ind]
    ...
    if not allow_singleton_pairs:
        nonsing_to_orig_map = np.array(list(range(len(y))))[np.invert(sing_ind)]
...
def dendromerge(X,y,score_threshold=None,preprocessing=log_correlation,clust_dist=cdist_log_Welchs,
                ...
                outlier_threshold_percentile=100,perform_community_detection=False):
    '''
    ...
    for i,j in itertools.combinations(list(range(Nc)),2):
        cache,Dc[i,j] = clust_dist(X,D,i,j,z,cache)
        Dc[j,i] = Dc[i,j]

    if save_prefix is not None: pickle.dump(Dc,open(save_prefix+'_Dc.pickle','w'))
    ...
    if perform_community_detection:
        ...
        def D_to_squareform_with_thresholding(D,thresh=None):
            indices = list(itertools.combinations(list(range(0,len(D))),2))
