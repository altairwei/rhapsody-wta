...
def annotate_reads(annotR1,
    ...
    if filtering_stats:
        ...
        filtering_stat_files = [filtering_stat for filtering_stat in filtering_stats.split(',') if filtering_stat != 'None']
    ...
    if sample_tags_version and subsample_tags:  # multiplexing and subsampling enabled
        if subsample_tags <= 0 or subsample_tags == 1:  # check valid number
            raise argparse.ArgumentError('--subsample-tags',
    ...
    output_header = build_output_header(assay, cur_time, sample, label_version, idx, num_bc,
    ...
    print('Start writing Read Annotation at', time.strftime("%Y-%m-%d %H:%M:%S"), file=sys.stderr, flush=True)
    for library_name, library_files in files_by_library.items():
        print(time.strftime("%Y-%m-%d %H:%M:%S"), 'Writing Read Annotation:', library_name, file=sys.stderr, flush=True)
        library_metrics = write_read_annotations_and_calculate_metrics(library_files, read_annot, valid_annot,
                                                                       valid_annot_trueno, assay, trueno, umi_option)
        metrics_by_library[library_name] = library_metrics
    print('Finish writing Read Annotation at', time.strftime("%Y-%m-%d %H:%M:%S"), file=sys.stderr, flush=True)
    ...
    utils.cleanup(['*_SeqMetrics.csv', '*Sorted_Valid_Reads.csv.*', '*_Annotation_Read.csv'], [], ['*Targeted_Sorted_Valid_Reads.csv', '*Targeted_Trueno_Valid_Reads.csv', '*Targeted_Valid_Reads.csv'])
...
def write_read_annotations_and_calculate_metrics(library, read_annot_fp, valid_annot_fp,
                                                 valid_annot_trueno_fp, assay, trueno, umi_option):
    """
    Args:
    ...
    with open(read_annot_fp, 'a') as f, \
        ...
        library_metrics = LibraryMetrics(trueno, assay, umi_option, filter_stats_fps, picard_quality_metrics_fps)
...
def write_read_stats(metrics_by_library, stats_combined, assay, seq_file, output_header):
    ...
    with open(seq_file, 'w') as f:
        ...
        filtering_header = ['Num_Reads_in_FASTQ', 'Num_Reads_After_Length_Filter', 'Pct_Len_Filter',
                            ...
                            'Num_Reads_After_Combined_Filter', 'Pct_Discarded_by_All_Filters']
        ...
        if assay == 'Targeted':
            seq_header.extend(('Reads_with_Correct_Start_Position', 'Pct_Correct_Start',
                               'Reads_Passing_Alignment_Length', 'Pct_Align_Len'))
        else:
        ...
        picard_tool_header = ['TOTAL_READS', 'PF_READS', 'READ_LENGTH', 'TOTAL_BASES', 'PF_BASES', 'Q20_BASES',
                              ...
                              'PF_Q20_EQUIVALENT_YIELD', 'PCT_Q30_BASES',]

        rb.writerow([col.title() for col in picard_tool_header])
        for lib_name, stats in metrics_by_library.items():
            if lib_name != 'BAM_file_stats':
                write_dict_row(stats['picard_stats'], lib_name, order=picard_tool_header)
        if 'BAM_file_stats' in metrics_by_library:
            write_dict_row(metrics_by_library['BAM_file_stats']['picard_stats'], 'BAM_file_stats', order=picard_tool_header)
        else:
            rb.writerow(['NA'] * len(picard_tool_header) + ['BAM_file_stats'])
        write_dict_row(stats_combined['picard_stats'], 'Combined_stats', order=picard_tool_header)
        ...
        trueno_header = ['Total_Reads', 'Useful_Reads', 'Pct_Useful', 'Reads_Assigned_to_Cells', 'Pct_Cellular',
                         ...
                         'Reads_Passing_Alignment_Length', 'Pct_Align_Len', 'Final_Useful_Reads_After_Subsampling']
...
def build_output_header(assay, start, sample, label_version, idx, num_bc, putative_cell_call, reference_panel_names, sample_tags_version, subsample_tags, bam_input, extra_seqs):
    """
