...
enrichdf <- gsea@result |>
  ...
  dplyr::mutate(Count = length(strsplit(core_enrichment, "/")[[1]])) |>
  dplyr::select(ID, Description, NES,
                Count, p.adjust, core_enrichment)
```
...
reactable::reactable(
  ...
  details = reactable::colDef(
    ...
    details = function(row_idx) {
      reactable::reactable(
        data.frame(Details = enrichdf[row_idx, "core_enrichment"]),
...
enrichplot::ridgeplot(showCategory = 100, gsea, label_format = 100) +
...
pathways <- GO_DATA$PATHID2EXTID |>
  ...
  tidyr::unnest(Gene) |>
  dplyr::filter(Gene %in% rownames(sce_raw)) |>
  dplyr::group_by(GO) |>
...
pathway_summed <- calcPathwayActivity(
  sce_raw, pathways, method = "sum",
  annotation = gsea@result)
```

```{r eval=FALSE}
pathway_gsva <- xfun::cache_rds(
  file = "sce_mock_pathway_gsva.rds",
  dir = "../results/ObjectCache/TrajectoryInference/",
  expr = calcPathwayActivity(
    sce_raw, pathways, method = "gsva",
    annotation = gsea@result)
)
```
...
GSVA 算出来的活性，与 Time ncorrs 的相关性似乎更高点，但没有本质性的差别。
...
```{r}
poscorr <- c(
  "GO:0002181", "GO:0006833", "GO:0042219", "GO:1901658")
```

```{r}
enrichplot::gseaplot2(gsea, subplots = 1:2, geneSetID = poscorr)
```
...
```{r}
enrichplot::gseaplot2(gsea, subplots = 1:2, geneSetID = negcorr)
```
...
patchwork::wrap_plots(piclist, ncol = 2)
```
...
patchwork::wrap_plots(piclist, ncol = 3)
```
