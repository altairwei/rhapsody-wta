# 3DPI 样品的差异状态分析 (Differential State analysis)
...
obj <- readRDS("../results/IntegratedAnalysis/3DPI-All/Seurat_Object_Combined.rds")
```
...
我们首先画一下 `X3DPI.PNR2-X3DPI.MOCK` 火山图：
...
然后看一下 `X3DPI.TR4-X3DPI.MOCK` 的火山图：
...
tbl_filtered <- ds_res$DESeq2 %>%
  rhapsodykit::diff_state_significant(0.01, 2) %>%
  .$table %>% .[["X3DPI.PNR2-X3DPI.MOCK"]]

de_gs_by_k <- purrr::map(tbl_filtered, "gene")

ds_res$DESeq2 %>%
  rhapsodykit::diff_state_significant(0.01, 2) %>%
  rhapsodykit::diff_state_pull("X3DPI.PNR2-X3DPI.MOCK", 1:15, "gene") %>%
...
ds_res$DESeq2 %>%
  rhapsodykit::diff_state_significant(0.01, 2) %>%
  rhapsodykit::diff_state_pull("X3DPI.TR4-X3DPI.MOCK", 1:15, "gene") %>%
  rhapsodykit::from_list_to_upset() %>%
  rhapsodykit::upset_dataframe() %>%
  rhapsodykit::upset_plot()
...
ds_res$DESeq2 %>%
  rhapsodykit::diff_state_significant(0.01, 2) %>%
  rhapsodykit::diff_state_pull("X3DPI.PNR2-X3DPI.TR4", 1:15, "gene") %>%
  rhapsodykit::from_list_to_upset() %>%
  rhapsodykit::upset_dataframe() %>%
  rhapsodykit::upset_plot()
```
...
plot_updown <- function(contrast) {
  up_list <- ds_res$DESeq2 %>%
    ...
    rhapsodykit::diff_state_significant(0.01, 2) %>%
    rhapsodykit::diff_state_pull(contrast, 1:15, "gene")
  ...
  rhapsodykit::upset_updown_regulated(up_list, down_list, n_sets = 10)
}

plot_updown("X3DPI.PNR2-X3DPI.MOCK") + ggplot2::ggtitle("3DPI PNR2 vs. MOCK")
plot_updown("X3DPI.TR4-X3DPI.MOCK") + ggplot2::ggtitle("3DPI TR4 vs. MOCK")
plot_updown("X3DPI.PNR2-X3DPI.TR4") + ggplot2::ggtitle("3DPI PNR2 vs. TR4")
```
      ...
      set.metadata = list(
        ...
        plots = list(
          list(
            ...
            alpha = 0.4,
            colors = c(
              "X3DPI.PNR2-X3DPI.MOCK" = "green",
              #"X3DPI.TR4-X3DPI.MOCK" = "navy",
              "X3DPI.PNR2-X3DPI.TR4" = "yellow"
            )
          )
        )
      )
...
```{r fig.height=7, fig.width=7}
compare_list <- list(
  "VCs_5.3DPI.TR4.MOCK" = ds_res$DESeq2 %>%
    rhapsodykit::diff_state_significant(0.01, 2) %>%
    rhapsodykit::diff_state_pull("X3DPI.TR4-X3DPI.MOCK", "Vascular Cells 5", "gene"),
  "VCs_5.3DPI.PNR2.MOCK" = ds_res$DESeq2 %>%
    rhapsodykit::diff_state_significant(0.01, 2) %>%
    rhapsodykit::diff_state_pull("X3DPI.PNR2-X3DPI.MOCK", "Vascular Cells 5", "gene"),
  "VCs_6.3DPI.TR4.MOCK" = ds_res$DESeq2 %>%
    rhapsodykit::diff_state_significant(0.01, 2) %>%
    rhapsodykit::diff_state_pull("X3DPI.TR4-X3DPI.MOCK", "Vascular Cells 6", "gene"),
  "VCs_6.3DPI.PNR2.MOCK" = ds_res$DESeq2 %>%
    rhapsodykit::diff_state_significant(0.01, 2) %>%
    rhapsodykit::diff_state_pull("X3DPI.PNR2-X3DPI.MOCK", "Vascular Cells 6", "gene")
)
...
p1 <- ds_res$DESeq2 %>%
  ...
  rhapsodykit::enrich_barplot(x = "Count", show_category = 20, title = "Vascular Cells 5 of 3DPI-TR4 vs. 3DPI-MOCK")

p2 <- ds_res$DESeq2 %>%
  rhapsodykit::diff_state_significant(0.01, 2) %>%
  rhapsodykit::diff_state_pull("X3DPI.PNR2-X3DPI.MOCK", "Vascular Cells 5", "gene") %>%
  rhapsodykit::enrichment_analysis(go_data) %>%
  rhapsodykit::enrich_barplot(x = "Count", show_category = 20, title = "Vascular Cells 5 of 3DPI-PNR2 vs. 3DPI-MOCK")

p3 <- ds_res$DESeq2 %>%
  rhapsodykit::diff_state_significant(0.01, 2) %>%
  rhapsodykit::diff_state_pull("X3DPI.PNR2-X3DPI.TR4", "Vascular Cells 5", "gene") %>%
  rhapsodykit::enrichment_analysis(go_data) %>%
  rhapsodykit::enrich_barplot(x = "Count", show_category = 20, title = "Vascular Cells 5 of 3DPI-PNR2 vs. 3DPI-TR4")
...
genes <- sig_genes %>%
  rhapsodykit::diff_state_pull("X3DPI.PNR2-X3DPI.MOCK", "Vascular Cells 5", "gene")
fc <- sig_genes %>%
  rhapsodykit::diff_state_pull("X3DPI.PNR2-X3DPI.MOCK", "Vascular Cells 5", "logFC")
names(fc) <- genes
...
gsea_df <- ds_res$DESeq2 %>%
  rhapsodykit::diff_state_pull("X3DPI.PNR2-X3DPI.MOCK", "Vascular Cells 5", c("gene", "logFC"))
