...
performDSCompareORA <- function(
  ...
  compare_list <- purrr::imap(clusters, function(clr, clr_name) {
      purrr::imap(contrasts, function(con, con_name) {
        df <- ds %>%
          rhapsodykit::diff_state_significant() %>%
...
clusterProfiler::goplot(enr1$enr)
```
...
rhapsodykit::enrich_cnetplot(enr1$enr, fold_change = enr1$fc, show_category = 30) +
rhapsodykit::enrich_cnetplot(enr2$enr, fold_change = enr2$fc, show_category = 30)
```
...
comp_enr <- performDSCompareORA(ds_res$DESeq2,
  contrasts = c(
    "PNR2 vs. MOCK - 1DPI" = "X1DPI.PNR2-X1DPI.MOCK",
    "TR4 vs. MOCK - 1DPI" = "X1DPI.TR4-X1DPI.MOCK"
  ),
  clusters = c(
    "VCs_5" = "VCs_5",
    "MCs_5" = "MCs_5"
...
```{r fig.height=15, fig.width=10, fig.keep="last"}
...
enrichplot::dotplot(comp_enr, showCategory = 40, label_format = 80) +
  ggplot2::theme(
...
comp_enr %>%
  enrichplot::pairwise_termsim() %>%
  enrichplot::emapplot(legend_n = 2, cex_line = 0.4)
```
...
```{r fig.height=10, fig.width=7, fig.keep="last"}
...
```{r enrich-compare, fig.height=12, fig.width=8, fig.keep="last"}
comp_enr <- performDSCompareORA(ds_res$DESeq2,
  contrasts = c(
    "PNR2 vs. MOCK - 1DPI" = "X1DPI.PNR2-X1DPI.MOCK",
    "TR4 vs. MOCK - 1DPI" = "X1DPI.TR4-X1DPI.MOCK"
