...
## Heatmap Stripe for Model

### Outer sheath

```{r fig.height=2, fig.width=1}
p <- plotImmunityHeatmap(
  se = lfcse[rownames(lfcse) %in% pathways$BXDs,
             lfcse$cell_types == "Outer sheath"],
  col_split_by = "contrast",
  col_anno_by = "times",
  show_row_dend = FALSE,
  show_col_anno_name = FALSE,
  show_heatmap_legend = FALSE,
  column_title = NULL,
  show_col_anno_legend = FALSE,
  simple_anno_size = grid::unit(3, "mm"),
  border = FALSE
)

plotpreview(p, width = 1, height = 2, ext = ".svg")
```

```{r fig.height=2, fig.width=1}
p <- plotImmunityHeatmap(
  se = lfcse[rownames(lfcse) %in% pathways$RLK,
             lfcse$cell_types == "Outer sheath"],
  col_split_by = "contrast",
  col_anno_by = "times",
  show_row_dend = FALSE,
...
```{r fig.height=2, fig.width=1}
p <- plotImmunityHeatmap(
  se = lfcse[rownames(lfcse) %in% unlist(pathways[c(
                "PhotosynPSEA", "PhotosynAnt", "PhotosynCfix")]),
             lfcse$cell_types == "Outer sheath"],
  col_split_by = "contrast",
  col_anno_by = "times",
  show_row_dend = FALSE,
...
```{r fig.height=2, fig.width=1}
p <- plotImmunityHeatmap(
  se = lfcse[rownames(lfcse) %in% pathways$NBLRR,
             lfcse$cell_types == "Parenchyma"],
  col_split_by = "contrast",
  col_anno_by = "times",
  show_row_dend = FALSE,
...
```{r fig.height=2, fig.width=1}
p <- plotImmunityHeatmap(
  se = lfcse[rownames(lfcse) %in% pathways$`General phenylpropanoid`,
             lfcse$cell_types == "Procambium"],
  col_split_by = "contrast",
  col_anno_by = "times",
  show_row_dend = FALSE,
  show_col_anno_name = FALSE,
...
p <- plotImmunityHeatmap(
  se = lfcse[rownames(lfcse) %in% RBOH$ID,],
  col_split_by = "cell_types",
  col_anno_by = c("times", "contrast"),
  show_row_dend = FALSE,
  show_row_names = TRUE,
  show_col_anno_name = FALSE,
  show_heatmap_legend = FALSE,
  ...
  row_labels = with(RBOH, {
    Name[match(rownames(lfcse)[rownames(lfcse) %in% ID], ID)]
  })
)
```
...
```{r fig.height=2.5, fig.width=9}
p <- plotImmunityHeatmap(
  se = lfcse[rownames(lfcse) %in% pathways$TaGST,],
  col_split_by = "cell_types",
  col_anno_by = c("times", "contrast"),
  show_row_dend = FALSE,
  show_col_anno_name = FALSE,
...
```{r fig.height=2.5, fig.width=9}
p <- plotImmunityHeatmap(
  se = lfcse[rownames(lfcse) %in% pathways$Polyamine,],
  col_split_by = "cell_types",
  col_anno_by = c("times", "contrast"),
  show_row_dend = FALSE,
  show_col_anno_name = FALSE,
