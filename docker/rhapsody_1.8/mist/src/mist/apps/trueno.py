...
from mist.apps.ResolveClusterAnalysis import run_clust_analysis, load_st
import re
...
def sampleTagAnalysis(DT1, DT2, DT3, DT4, matrix_file, name, output_header, cell_annot_st,
                      sample_tag_list, true_cells, tag_names, assay, cell_order, gene_list, wta_only):
    """
    ...
    # internal outputs
    st_details_dir = 'SampleTag'
    if not os.path.exists(st_details_dir):
        os.mkdir(st_details_dir)
...
def call_sample_tag(counts_each_cell, output_header, SampleTag_status_out, SampleTag_noiseTrend_out,
    ...
    if len(total_count_x)>0:
        ...
        trend_z = np.polyfit(total_count_x, noise_count_y, 1)
