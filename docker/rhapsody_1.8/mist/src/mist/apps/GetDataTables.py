...
def get_data_tables(_args=None):
    ...
    DT1 = "{}_RSEC_ReadsPerCell_Sparse.csv.gz".format(name)
    DT2 = "{}_DBEC_ReadsPerCell_Sparse.csv.gz".format(name)
    DT3 = "{}_RSEC_MolsPerCell_Sparse.csv.gz".format(name)
    DT4 = "{}_DBEC_MolsPerCell_Sparse.csv.gz".format(name)
    ...
    DT1_all = "{}_RSEC_ReadsPerCell_Unfiltered_Sparse.csv.gz".format(name)
    DT2_all = "{}_DBEC_ReadsPerCell_Unfiltered_Sparse.csv.gz".format(name)
    DT3_all = "{}_RSEC_MolsPerCell_Unfiltered_Sparse.csv.gz".format(name)
    DT4_all = "{}_DBEC_MolsPerCell_Unfiltered_Sparse.csv.gz".format(name)
    ...
    for sf in split_file_list:
        z = pd.read_csv(sf, header=None,
                        names=['cell', 'gene', 'reads', 'dbec_reads', 'raw_mols', 'rsec_mols', 'dbec_mols'],
                        index_col=[0, 1])
        genes.update(z.index.levels[1])   # compute the full set of genes for the final merge
        # total_reads = z.groupby(z.index.get_level_values(0))['reads'].apply(sum).sort_values(ascending=False)
        total_reads = z.groupby(z.index.get_level_values(0))['dbec_reads'].apply(sum).sort_values(ascending=False)
    ...
    # pass expressed Gene list to metrics node
    with open(expressed_gene_list, 'w') as f:
        f.write('\n'.join(genes))
    ...
    if assay == 'WTA':   # if WTA use all expressed genes
        full_gene_list += sorted(genes)
        full_gene_list = sorted(full_gene_list)
    ...
    if label_version not in (3, 4):
        ...
        z = pd.read_csv(cell_annot, header=None,
                    names=['cell', 'gene', 'reads', 'dbec_reads', 'raw_mols', 'rsec_mols', 'dbec_mols'],
                    dtype={'reads': np.int32, 'dbec_reads': np.int32, 'raw_mols': np.int32, 'rsec_mols': np.int32, 'dbec_mols': np.int32},
                    index_col=[0, 1])
        ...
        cell_order_dict['Unfiltered'], gene_list = writeDataTables(merged_table_file_all, full_gene_list, DT1_all, DT2_all, DT3_all, DT4_all, matrix_file_all, output_header, putative_cell_call, wta_only)
        ...
        if expected_num_cell is not None:
            ...
            num_cell = orig_alg.main(cell_read_counts.values, num_cell_image, name, output_header=output_header, stats_file=algostats_file, do_plot=True)
            cell_list = cell_read_counts[:num_cell]
            ...
            final_cells = improved_alg.main(cell_read_counts, num_cell_image, name, split_file_list,
                                        num_genes_in_panel, output_header, algostats_file, assay)
    ...
    cell_order_dict['Filtered'], gene_list = writeDataTables(merged_table_file, full_gene_list, DT1, DT2, DT3, DT4, matrix_file, output_header, putative_cell_call, wta_only)
    ...
    if trueno_run:
        ...
        df_sample_tag_calls = trueno.sampleTagAnalysis(DT1, DT2, DT3, DT4, matrix_file, name, output_header, cell_annot_tags, sample_tag_list, list(cell_list.index),
                                 tag_names, assay, cell_order_dict['Filtered'], full_gene_list, wta_only)

        # load sample tag data
        z_tags = pd.read_csv(cell_annot_tags, header=None, names=['cell', 'tag', 'reads', 'dbec_reads', 'raw_mols', 'rsec_mols', 'dbec_mols'], index_col=[0,1])
        tag_reads = z_tags['reads'].reset_index(level='tag')

        snr_folder = path.join(os.getcwd(), 'SampleTag')
        if not os.path.exists(snr_folder):
            os.mkdir(snr_folder)

        # calculate sample tag signal to noise ratio
        trueno.calculate_trueno_snr(tag_reads, df_sample_tag_calls, final_cells, snr_folder, plot_hist = False)

    # Calculate AbSeq metrics and graphs
    logging.info('Generating AbSeq Metrics')

    logging.info('Loading cell_annot_pAbo into memory...')
    z_pabo = pd.read_csv(cell_annot_pAbo, header=None, names=['cell', 'gene', 'reads', 'dbec_reads', 'raw_mols', 'rsec_mols', 'dbec_mols'], index_col=[0,1])
...
def writeDataTables(in_file, ref_genes, ofreads, ofdbecreads, ofrsecmols, ofdbecmols, matrix_file, output_header, putative_cell_call, wta_only):
    ...
    drop_list = []
    if label_version not in (3, 4):
        # sort by total read counts by cell in descending order (depending on what data used for cell calling, different subset of genes may be used)
        if putative_cell_call == PutativeCellCall.mrna_only:
    ...
    for output_fp, column_name in [(ofreads, 'RSEC_Reads'),
        ...
        if wta_only and 'Unfiltered' in output_fp and 'DBEC' in column_name:
            continue
    ...
    # maintain the original cell ordering, and order secondarily by gene, for the sparse data table
