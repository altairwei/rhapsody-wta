...
plot_cell_population_by_time <- function(res) {
  # Use BCa as CI
  ...
  ggplot2::ggplot(df_to_plot,
    ...
    ggplot2::facet_wrap(~cellTypes, scales = "free_y", ncol = 3) +
    ggplot2::theme_bw() +
    NULL
}

plot_cell_population_by_time(res_scDC_noClust)
```
...
```{r fig.height=10, fig.width=10}
plot_bootstrap_distribution <- function(res) {
  # Use BCa as CI
  df_to_plot <- res$results %>%
    tibble::as_tibble() %>%
    dplyr::filter(method == "BCa") %>%
  ...
  ggplot2::ggplot(df_to_plot,
    ...
    ggplot2::geom_line(
      ...
      position = ggplot2::position_dodge(width = 0.4)) +
    ggplot2::scale_y_continuous(labels = scales::percent) +
    ggplot2::facet_wrap(~cellTypes, scales = "free_y") +
    ggplot2::theme_bw() +
    NULL
}

plot_bootstrap_distribution(res_scDC_noClust)
```
