...
data.frame(time_ncorrs = sce_mock$time_ncorrs,
           cellType = forcats::fct_relevel(
             droplevels(sce_mock$cellType),
             "Me_γ", "MPV", "Va_δ", "Me_δ", "BS")) |>
  ggplot2::ggplot(ggplot2::aes(x = time_ncorrs, fill = cellType)) +
  ggplot2::geom_density(position = "stack") +
  ggplot2::geom_vline(xintercept = cnaRes$fdr_5p_t) +
  ggplot2::geom_vline(xintercept = -cnaRes$fdr_5p_t) +
  NULL
```
...
data.frame(time_ncorrs = sce_mock$time_ncorrs,
           cellType = forcats::fct_relevel(
             droplevels(sce_mock$cellType),
             "Me_γ", "MPV", "Va_δ", "Me_δ", "BS")) |>
  ggplot2::ggplot(ggplot2::aes(x = time_ncorrs, y = cellType, fill = cellType)) +
...
```{r}
cnaRes$NAM_embeddings |> head()
```

```{r}
ncorrs <- as(cnaRes$NAM_embeddings, "dgCMatrix")
cormtx <- qlcMatrix::corSparse(ncorrs, t(logcounts(sce_mock)))
rownames(cormtx) <- colnames(ncorrs)
colnames(cormtx) <-  rownames(logcounts(sce_mock))
cormtx <- t(cormtx)
head(cormtx)
```
...
df_to_plot <- data.frame(
  ...
  Time = stringr::str_extract(rownames(cnaRes$NAM_loadings), "\\dDPI-MOCK")
)
